...
int main(int argc, const char * argv[]) {
    ...
    char N_char[MSGSIZE],g_char[MSGSIZE],h_char[MSGSIZE],w_char[MSGSIZE],u_char[MSGSIZE],Rd_char[MSGSIZE];
    ...
    recv(sClient,h_char, MSGSIZE, 0);
    recv(sClient,w_char, MSGSIZE, 0);
    recv(sClient,u_char, MSGSIZE, 0);
    close(sClient);
    ...
    string Cloud_Parameter_path_str = "xxxx";
    ...
    write_ptr<<w_str<<endl;
    write_ptr<<u_str<<endl;
    write_ptr.close();
    ...
    element_pow_zn(sen,g,sk);
    
    char pkd_char[MSGSIZE];
    element_snprint(pkd_char, 2*parame_length, sen);
    string pkd_str = ExtraG1(pkd_char);
    append(pkd_str, pkd_char);
    ...
    sendNo=send(sClient,pkd_char, MSGSIZE, 0);
    close(sClient);
    ...
    string Rd_path_str = "xxx";
    
    string sk_str = mpz_get_str(NULL,10,sk_t);
    write_ptr.open(Rd_path_str,ios::out);
    write_ptr<<sk_str<<endl;
    write_ptr<<Rd_str<<endl;
    write_ptr.close();
    
    char x_char[MSGSIZE],theta_char[MSGSIZE];
    ...
    recv(sClient,x_char, MSGSIZE, 0);
    recv(sClient,theta_char, MSGSIZE, 0);
    close(sClient);
    string x_str = InfoExtra(x_char);
    string theta_str = InfoExtra(theta_char);
    string signKey_Parameter_path_str = "xxxx";
    
    write_ptr.open(signKey_Parameter_path_str,ios::out);
    write_ptr<<x_str<<endl;
    write_ptr<<theta_str<<endl;
    write_ptr.close();
